...
<div class="main-nav--div flex gap-x-6 items-center 4xl:gap-x-10">
  ...
  if ($menu_items) :
  ?>
    <ul class="main-nav--ul flex gap-x-6 3xl:gap-x-8 4xl:gap-x-6">
      <?php foreach ($menu_items as $menu_id => $menu) : ?>
        ...
        ?>
          <li class="<?php echo $li_class ?>">
            <a href="<?php echo $menu_item['url'] ?>" target="<?php echo $menu_item['target'] ?>" class="inline-block text-white font-semibold text-lg uppercase whitespace-nowrap py-8 px-4 border-t-[6px] border-transparent hover:border-brand-red transition duration-300"><?php echo $menu_item['title'] ?></a>
                              ...
                              <?php foreach ($submenu_items as $submenu_id => $item) : ?>
                                ...
                                <li>
                                  <a href="<?php echo $submenu_item['url'] ?>" class="<?php echo $submenu_class ?>" data-target="<?php echo $data_target ?>">
                                    ...
                                    <?php if ($featured) : ?>
                                      <?php echo tkw_icon(array('icon' => 'chevron', 'group' => 'utilities', 'size' => '18', 'class' => 'text-brand-orange absolute right-0 top-4')); ?>
                                    <?php endif; ?>
                                  </a>
                                </li>
                              <?php endforeach; ?>
                              ...
                              <?php if ($featured) : ?>
                                <div id="<?php echo $article_id; ?>" class="menu-article" tabindex="-1">
                                  <div class="flex flex-col gap-y-4">
                                    ...
                                    <?php endif; ?>
                                    <?php if ($title) : ?>
                                      <h2 class="text-[28px] font-bold text-brand-blue line-clamp-2"><?php echo $title ?></h2>
                                    <?php endif; ?>
                                    <?php if ($description) : ?>
  ...
  if ($phone) :
  ?>
    <a href="<?php echo $phone['url'] ?>" target="<?php echo $phone['target'] ?>" class="flex px-6 py-3 gap-x-4 text-white font-medium border border-solid border-white rounded-full bg-transparent hover:bg-brand-red hover:border-brand-red transition duration-200">
